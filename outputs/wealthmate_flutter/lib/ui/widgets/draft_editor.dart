...
class _DraftEditorDialogState extends State<DraftEditorDialog> {
  ...
  void initState() {
    ...
    categoryId = LedgerRules.validCategoryIdForType(
        widget.state, type, draft.categoryId);
      ...
      content: SingleChildScrollView(
        ...
        SegmentedButton<TransactionType>(
          ...
          onSelectionChanged: (value) => setState(() {
            type = value.first;
            final nextCategory =
                LedgerRules.defaultCategoryIdForType(widget.state, type);
            categoryId = nextCategory.isEmpty ? null : nextCategory;
          }),
        ),
        const SizedBox(height: 10),
        DropdownButtonFormField<String>(
            key: ValueKey('draft-category-${type.name}-$categoryId'),
        ...
        DropdownButtonFormField<String>(
            initialValue:
                accounts.any((item) => item.id == accountId) ? accountId : null,
        ...
        TextField(
          ...
          decoration:
              const InputDecoration(labelText: '日期', hintText: 'YYYY-MM-DD'),
        ),
        const SizedBox(height: 10),
        TextField(
  ...
  void _save() {
    ...
    Navigator.pop(
        ...
        AgentDraft(
          ...
          fromAccountId: widget.draft.fromAccountId,
          toAccountId: widget.draft.toAccountId,
        ));
  }
}
