...
开发/测试时，Windows 客户端可以连接本机 `http://127.0.0.1:18000`；手机不能使用 `127.0.0.1`，局域网联调可以使用局域网地址。正式环境必须由发布环境注入 HTTPS API 域名。本项目当前没有提供最终正式域名，不得把局域网地址、IP 或占位域名用于正式 V1.0.3。
